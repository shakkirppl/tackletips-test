                    ...
                    <div class="card">
                        <div class="card-body">
                         ...
                         <form action="{{url('update-values')}}" method="post" enctype="multipart/form-data">
                            @foreach($attributes_value as $attributes_value)
                           {{csrf_field()}}

                            <div class="form-group">
                               <input type="hidden" name="attr_value_id" value="{{ $attributes_value->attribute_value_id}}">
                                <label>Attribute Name</label>
                                <select class="form-control" name="attr_name" onchange="val()" id="select_id">
                                   <?php 
                            ...
                            <div class="form-group">
                                <label>Attribute Value</label>
                                 ...
                                 <input type="text" name="attr_val" value="{{$attributes_value->attribute_value }}" placeholder="Enter the value" class="form-control">
                                <i class="form-group__bar"></i>
                                 
                            </div>
                            
                             <div style="display: none;" id="inp_hiden">
                              <div class="form-group">
                                <label>Enter a Hexcode (Example:#e6194b for color Red.)</label>
                                  <input type="text" value="{{$attributes_value->attribute_color_code}}" name="hex_val_ed" placeholder="Enter corresponding Hexcode" class="form-control">
                                <i class="form-group__bar"></i>
                              </div>
                            </div>
  ...
  <!-- Vendors -->
        ...
        function val() {
          d = document.getElementById("select_id").value;
            if (d=='2') {
              document.getElementById("inp_hiden").style.display="block";
            }else{
              document.getElementById("inp_hiden").style.display="none";
            }
        }
        </script>
