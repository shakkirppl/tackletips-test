                         ...
                         <form action="{{url('update-category')}}" method="post" enctype="multipart/form-data">
                           ...
                           {{csrf_field()}}
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="category" class="form-control" data-toggle="tooltip" data-placement="bottom" title="Category Name" value="{{ $ed_cat->name}}">
                                ...
                                <select class="form-control" name="parent_category_ed" data-toggle="tooltip" title="Parent category">
                                    <?php 
                                        $parnt_nme = DB::table('categories')
                                                    ->where('id','=',$ed_cat->parent_id)
                                                    ->get();
                                    ?>
                                    @foreach($parnt_nme as $parnt_nme)
                                    <option selected="" value="{{ $parnt_nme->id }}" style="color: #32c787;">{{ $parnt_nme->name }}</option>
                                    @endforeach
